    ...
    <div class="container-fluid p-0">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h3 class="fw-bold mb-0">
                <i class="bi bi-people me-2"></i>{{ __('tenant::users.create_group') ?? 'Create Group' }}
            </h3>
        ...
        <form action="{{ route('tenant.settings.users.groups.store') }}" method="POST" id="groupForm">
            @csrf
            <div class="row g-4">
                <div class="col-md-5">
                        ...
                        <div class="card-body p-4">
                            ...
                            <div class="form-check form-switch p-3 bg-light rounded-3">
                                <input class="form-check-input ms-0 me-2" type="checkbox" name="allow_ticket_assign"
                                    id="allowTicketAssign" value="1" {{ old('allow_ticket_assign') ? 'checked' : '' }}>
                                                ...
                                                <tr data-id="{{ $member['id'] }}" data-type="{{ $member['type'] }}">
                                                    ...
                                                    <td>
                                                        <input type="hidden" name="members[{{ $index }}][id]"
                                                            value="{{ $member['id'] }}">
                                                        <input type="hidden" name="members[{{ $index }}][type]"
                                                            value="{{ $member['type'] }}">
                                                        <input type="hidden" name="members[{{ $index }}][name]"
                                                            value="{{ $member['name'] }}">
                                                        <button type="button" class="btn btn-sm text-danger remove-member"><i
...
@push('scripts')
    <script>
        ...
        let memberCount = {{ old('members') ? count(old('members')) : 0 }};
