        ...
        <form action="{{ route('tenant.settings.users.profiles.update', $profile->profileid) }}" method="POST"
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                ...
                <div class="card-body p-4">
                    <div class="mb-3">
                        <label class="form-label fw-bold">{{ __('tenant::users.profile_name') ?? 'Profile Name' }} <span
                                class="text-danger">*</span></label>
                        <input type="text" name="profilename" class="form-control rounded-3"
                            value="{{ old('profilename', $profile->profilename) }}" required>
                    ...
                    <div class="mb-0">
                        ...
                        <textarea name="description" class="form-control rounded-3"
                            rows="2">{{ old('description', $profile->description) }}</textarea>
                    </div>
                </div>
            </div>
                        ...
                        <select name="copy_from_profile" class="form-select rounded-3" id="copyFromProfile">
                            ...
                            @if(isset($profiles))
                                @foreach($profiles as $p)
                                    @if($p->profileid != $profile->profileid && (!isset($p->directly_related_to_role) || $p->directly_related_to_role != 1))
                                ...
                                <tr>
                                    ...
                                    <th class="text-center" style="width: 13%;">
                                        <div class="d-flex flex-column align-items-center">
                                            ...
                                            <span><i
                                                    class="bi bi-plus-circle me-1"></i>{{ __('tenant::users.create') }}</span>
                                        </div>
                                    </th>
                                    <th class="text-center" style="width: 13%;">
                                        <div class="d-flex flex-column align-items-center">
                                    ...
                                    <th class="text-center" style="width: 13%;"><i
                                            class="bi bi-gear me-1"></i>{{ __('tenant::users.tools') }}</th>
                                </tr>
                            </thead>
                                        ...
                                        <tr data-module-id="{{ $module->tabid }}">
                                            ...
                                            <td class="text-center">
                                                <input type="checkbox" class="form-check-input permission-checkbox"
                                                    name="permissions[{{ $module->tabid }}][view]" data-permission="view"
                                                    data-module-id="{{ $module->tabid }}" value="1" {{ ($modulePrivs['view'] ?? false) ? 'checked' : '' }}>
                                            </td>
                                            <td class="text-center">
                                                <input type="checkbox" class="form-check-input permission-checkbox"
                                                    name="permissions[{{ $module->tabid }}][create]" data-permission="create"
                                                    data-module-id="{{ $module->tabid }}" value="1" {{ ($modulePrivs['create'] ?? false) ? 'checked' : '' }}>
                                            </td>
                                            <td class="text-center">
                                                <input type="checkbox" class="form-check-input permission-checkbox"
                                                    name="permissions[{{ $module->tabid }}][edit]" data-permission="edit"
                                                    data-module-id="{{ $module->tabid }}" value="1" {{ ($modulePrivs['edit'] ?? false) ? 'checked' : '' }}>
                                            </td>
                                            <td class="text-center">
                                                <input type="checkbox" class="form-check-input permission-checkbox"
                                                    name="permissions[{{ $module->tabid }}][delete]" data-permission="delete"
                                                    data-module-id="{{ $module->tabid }}" value="1" {{ ($modulePrivs['delete'] ?? false) ? 'checked' : '' }}>
                                            </td>
                                            <td class="text-center">
            ...
            <div class="d-flex justify-content-end gap-2 mb-4">
                <a href="{{ route('tenant.settings.users.profiles.index') }}"
...
@push('scripts')
    <script>
        // Pre-initialize data for edit mode
